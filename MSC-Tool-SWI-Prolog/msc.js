...
async function dependency_graph() {
	asy.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	pp.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	co.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	mb.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	onen.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	nn.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";
	rsc.innerHTML = "<span style='color: RED;'><b>WAIT</b></span>";

	dep_graph = new DependencyGraph(nodes, draw_edges, events); // pass highest node id

	await dep_graph.initalize();
	...
	for (let i = 0; i < com_name.length; i++) {
		let [is_valid, problem] = await is_com[i].call(dep_graph);  // !IMPORTANT to pass the context to call(),  
		...
		if (!is_valid) {
			for (let j = i+1; j < com_name.length; j++) {com_box[j].innerHTML = "<span style='color: RED;'><b>NO</b></span>"}
...
function parse_problem(com_name, problem) {
	...
	function change_color(message) {
		...
		for (let i = 0; i < nodes.length; i++) {
			...
			if ((src == -1 || trg == -1 ) && ("m".concat(nodes[i].m)).localeCompare(message)==0){
		...
		drawCircle(ctx, src.x, src.y, R, "red", STROKE);
		drawCircle(ctx, trg.x, trg.y, R, "coral", STROKE);
		let xr = -1;
		let yr = -1;
		if (trg.unmatched) {
		...
		drawArrow(ctx, src.x, src.y, xr, yr, "#000", STROKE, trg.unmatched); 
	}
