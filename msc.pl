...
nn(A) :- \+asy(_), nn_rel(X, X, A).

%---RSC---

crown_rel(s(X,Y,M1), s(Z,N,M2)) :- match(s(Z,N,M2), R2), rel(s(X,Y,M1), R2), M1\=M2.

crown_rel_transitive(X, Y) :- crown_rel_transitive(X, Y, _).
